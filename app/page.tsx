...
export default function Home() {
  ...
  return (
    <main style={{ fontFamily: "'Georgia', serif", background: '#0F0608', color: '#F5EDE8', overflowX: 'hidden' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, padding: '1.2rem 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(15,6,8,0.92)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(180,60,80,0.2)' }}>
        <span style={{ fontFamily: 'Georgia, serif', fontSize: '1.4rem', color: '#E8C4B8', letterSpacing: '0.05em' }}>La Mañana</span>
        <div style={{ display: 'flex', gap: '2.5rem' }}>
          {[{ label: 'Nosotros', id: 'nosotros' }, { label: 'Menú', id: 'menu' }, { label: 'Promociones', id: 'promociones' }, { label: 'Galería', id: 'galeria' }, { label: 'Reservas', id: 'reservas' }].map(item => (
            <a key={item.id} href={`#${item.id}`} onClick={(e) => { e.preventDefault(); document.getElementById(item.id)?.scrollIntoView({ behavior: 'smooth' }) }}
      ...
      <section style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '2rem', maxWidth: '800px' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="#reservas" onClick={(e) => { e.preventDefault(); document.getElementById('reservas')?.scrollIntoView({ behavior: 'smooth' }) }} style={{ background: '#8B1A2E', color: '#F5EDE8', padding: '0.9rem 2.2rem', textDecoration: 'none', fontSize: '0.85rem', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer' }}>Reservar mesa</a>
            <a href="#menu" onClick={(e) => { e.preventDefault(); document.getElementById('menu')?.scrollIntoView({ behavior: 'smooth' }) }} style={{ background: 'transparent', color: '#F5EDE8', padding: '0.9rem 2.2rem', textDecoration: 'none', fontSize: '0.85rem', letterSpacing: '0.1em', textTransform: 'uppercase', border: '1px solid rgba(245,237,232,0.25)', cursor: 'pointer' }}>Ver menú</a>
          </div>
        </div>
      </section>
      ...
      <section id="nosotros" style={{ padding: '7rem 3rem', background: '#130508', borderTop: '1px solid rgba(180,60,80,0.15)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'center' }}>
          <div style={{ position: 'relative' }}>
            <img src="https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=900&q=80" alt="Interior del café" style={{ width: '100%', height: '480px', objectFit: 'cover', borderLeft: '3px solid #8B1A2E', display: 'block' }} />
              ...
              <div key={promo.titulo} style={{ background: '#0F0608', border: '1px solid rgba(196,80,106,0.3)', overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ padding: '1.75rem' }}>
                  ...
                  <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid rgba(180,60,80,0.2)' }}>
                    <a href="#reservas" onClick={(e) => { e.preventDefault(); document.getElementById('reservas')?.scrollIntoView({ behavior: 'smooth' }) }} style={{ fontSize: '0.78rem', color: '#C4506A', textDecoration: 'none', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer' }}>Quiero esta promo →</a>
                  </div>
                </div>
              </div>
            ))}
          ...
          <form onSubmit={handleReserva}>
            <div style={{ marginBottom: '1.25rem' }}>
              ...
              <input required type="text" placeholder="Tu nombre" value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })}
                style={{ width: '100%', padding: '0.8rem 1rem', background: '#1A060C', border: '1px solid rgba(180,60,80,0.25)', color: '#F5EDE8', fontSize: '0.9rem', fontFamily: 'Georgia, serif', outline: 'none' }} />
            </div>
            <div style={{ marginBottom: '1.25rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.25rem' }}>
              <div>
                ...
                <input required type="date" value={form.fecha} onChange={e => setForm({ ...form, fecha: e.target.value })}
                  style={{ width: '100%', padding: '0.8rem 1rem', background: '#1A060C', border: '1px solid rgba(180,60,80,0.25)', color: '#F5EDE8', fontSize: '0.9rem', outline: 'none' }} />
              </div>
              <div>
            ...
            <div style={{ marginBottom: '1.25rem' }}>
              ...
              <textarea placeholder="Alergias, ocasión especial, preferencias..." rows={4} value={form.comentario} onChange={e => setForm({ ...form, comentario: e.target.value })}
                style={{ width: '100%', padding: '0.8rem 1rem', background: '#1A060C', border: '1px solid rgba(180,60,80,0.25)', color: '#F5EDE8', fontSize: '0.9rem', fontFamily: 'Georgia, serif', outline: 'none', resize: 'vertical' }} />
            </div>
            {enviado ? (
